...
RCT_EXPORT_METHOD(covertToBlackAndWhite:(NSString *)imageAsBase64 callback:(RCTResponseSenderBlock)callback) {
  //1. convert to UIImage to pass to make Mat
   UIImage* image = [self decodeBase64ToImage:imageAsBase64];
  
  //2. converting UIImage to OpenCV format - Mat
  //cv::Mat matImage = [self convertUIImageToCVMat:image];
  cv::Mat matImage;
  UIImageToMat(image,matImage,true);
  
  //3. converting image's color space (RGB) to grayscale
  cv::Mat matImageGrey;
  cv::cvtColor(matImage, matImageGrey, cv::COLOR_BGR2GRAY);
  
  
  UIImage *dst2Image = MatToUIImage(matImageGrey);
   NSData *imageData = UIImagePNGRepresentation(dst2Image);
  NSString * base64String = [imageData base64EncodedStringWithOptions:0];
  
  id objects[] = {base64String };
  NSUInteger count = sizeof(base64String) / sizeof(id);
  NSArray *dataArray = [NSArray arrayWithObjects:objects
                                           count:count];
